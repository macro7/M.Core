...
namespace M.Core.Utils
{
    ...
    public static class FileHelper
    {
        ...
        /// 从文件读取 Stream 再压缩
        /// </summary>
        public static Stream CompressFileToStream(string fileName)
        {
            // 打开文件
            FileStream fileStream = new FileStream(fileName, FileMode.Open, FileAccess.Read, FileShare.Read);
            // 读取文件的 byte[]
            ...
            // 把 byte[] 转换成 Stream
            MemoryStream outStream = new MemoryStream();
            using (MemoryStream intStream = new MemoryStream(bytes))
            {
                using (GZipStream Compress =
                    new GZipStream(outStream,
                    CompressionMode.Compress))
        ...
        /// 将 Stream 解压写入文件
        /// </summary>
        public static void DecompressStreamToFile(MemoryStream stream, string fileName)
        {
            // 把 Stream 转换成 byte[]
            MemoryStream compressedStream = new MemoryStream(stream.ToArray());
            using (MemoryStream outStream = new MemoryStream())
            {
                using (GZipStream Decompress = new GZipStream(compressedStream,
                        CompressionMode.Decompress))
        ...
        public static void SetAttributes(string fileName, bool IsReadOnly)
        {
            try
            {
                System.IO.File.SetAttributes(fileName, IsReadOnly ? System.IO.FileAttributes.ReadOnly : System.IO.FileAttributes.Normal);
            }
            catch (Exception e)
            {
                Logger.WriteErrorLog(e);
        ...
        public static void SetAttributes(string fileName, System.IO.FileAttributes fileAttributes)
        {
            try
            {
