    ...
    public class UCTrackBar : Control
    {
        ...
        public float Value
        {
            ...
            set
            {
                ...
                var v = (float)Math.Round(value, dcimalDigits);
                if (m_value == v)
                {
                    return;
                }
        ...
        public UCTrackBar()
        {
            ...
            MouseDown += UCTrackBar_MouseDown;
            MouseMove += UCTrackBar_MouseMove;
            MouseUp += UCTrackBar_MouseUp;
        ...
        void UCTrackBar_MouseDown(object sender, MouseEventArgs e)
        {
            ...
            {
                blnDown = true;
                Value = minValue + (e.Location.X / (float)Width) * (maxValue - minValue);
                ShowTips();
            }
        }
        /// <summary>
        ...
        private void ShowTips()
        {
            if (isShowTips)
            {
                ...
                var p = PointToScreen(new Point((int)m_trackRectangle.X, (int)m_trackRectangle.Y));
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            g.SetGDIHigh();
            m_lineRectangle = new RectangleF(lineWidth, (Size.Height - lineWidth) / 2, Size.Width - lineWidth * 2, lineWidth);
            ...
            GraphicsPath valueLine = ControlHelper.CreateRoundedRectanglePath(new RectangleF(lineWidth, (Size.Height - lineWidth) / 2, ((m_value - minValue) / (maxValue - minValue)) * m_lineRectangle.Width, lineWidth), 5);
            g.FillPath(new SolidBrush(m_valueColor), valueLine);

            m_trackRectangle = new RectangleF(m_lineRectangle.Left - lineWidth + (((m_value - minValue) / (maxValue - minValue)) * (Size.Width - lineWidth * 2)), (Size.Height - lineWidth * 2) / 2, lineWidth * 2, lineWidth * 2);
