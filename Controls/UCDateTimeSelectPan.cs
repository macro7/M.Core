...
namespace M.Core.Controls
{
    ...
    public partial class UCDateTimeSelectPan : UserControl
    {
        ...
        private void SetSelectType(UCButton btn)
        {
            try
            {
        ...
        {
            List<KeyValuePair<string, string>> lstSource = new List<KeyValuePair<string, string>>();
            int intYear = panTime.Source[0].Key.ToInt().Value - panTime.Source[0].Key.ToInt().Value % 30 - 30;
            panTime.SuspendLayout();
            panTime.Row = 5;
            panTime.Column = 6;
            for (int i = 0; i < 30; i++)
        ...
        {
            SelectedTimeEvent?.Invoke(m_dt, null);
        }
