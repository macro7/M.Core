    ...
    public partial class UCListView : UserControl
    {
        ...
        public UCPagerControlBase Page
        {
            ...
            set
            {
                ...
                if (value != null)
                {
                    ...
                    panPage.Controls.Add(m_page);
                    GetCellCount();
                    DataSource = m_page.GetCurrentSource();
                }
                else
                {
        ...
        public void ReloadSource()
        {
            try
            {
                ...
                {
                    for (int i = panMain.Controls.Count - 1; i >= 0; i--)
                    {
                ...
                int intCount = Math.Min(((IList)m_dataSource).Count, panMain.Controls.Count);

                for (int i = 0; i < intCount; i++)
                {
                    ((IListViewItem)panMain.Controls[i]).DataSource = ((IList)m_dataSource)[i];
                    if (m_selectedSource.Contains(((IList)m_dataSource)[i]))
                    {
                        ((IListViewItem)panMain.Controls[i]).SetSelected(true);
                    }
                    else
                    {
                        ((IListViewItem)panMain.Controls[i]).SetSelected(false);
                    }
                    panMain.Controls[i].Visible = true;
                }

                for (int i = panMain.Controls.Count - 1; i >= intCount; i--)
                {
        ...
        public void ReloadGridStyle()
        {
            ...
            Form frmMain = FindForm();
            if (frmMain != null && !frmMain.IsDisposed && frmMain.Visible && Visible)
            {
                GetCellCount();
                try
                {
                    ControlHelper.FreezeControl(this, true);
                    if (panMain.Controls.Count < m_intCellCount)
                    {
                        int intControlsCount = panMain.Controls.Count;
                    ...
                    else if (panMain.Controls.Count > m_intCellCount)
                    {
                        int intControlsCount = panMain.Controls.Count;
        ...
        private void GetCellCount()
        {
            ...
            int intXCount = (panMain.Width - 10) / (item.Width + 10);
            m_intCellWidth = item.Width + ((panMain.Width - 10) % (item.Width + 10)) / intXCount;

            int intYCount = (panMain.Height - 10) / (item.Height + 10);
            m_intCellHeight = item.Height + ((panMain.Height - 10) % (item.Height + 10)) / intYCount;
            ...
            if (Page == null)
            {
                ...
                else
                {
                    if (((IList)m_dataSource).Count > intCount)
                    {
                        ...
                        m_intCellWidth = item.Width + ((panMain.Width - 10 - 20) % (item.Width + 10)) / intXCount;
                    }
