    ...
    public partial class UCPagerControl2 : UCPagerControlBase
    {
        ...
        private void ReloadPage()
        {
            try
            {
                ...
                for (int i = 0; i < 9; i++)
                {
                    UCButton c = (UCButton)tableLayoutPanel1.Controls.Find("p" + (i + 1), false)[0];
                    if (i >= lst.Count)
                    {
                        c.Visible = false;
                    }
        ...
        {
            PageIndex = (sender as UCButton).BtnText.ToInt().Value;
            StartIndex = (PageIndex - 1) * PageSize;
            ReloadPage();
            var s = GetCurrentSource();
        ...
        private void btnToPage_BtnClick(object sender, EventArgs e)
        {
            ...
            {
                PageIndex = txtPage.InputText.ToInt().Value;
                StartIndex = (PageIndex - 1) * PageSize;
                ReloadPage();
                var s = GetCurrentSource();
